...
def collapse_layers(img):
    """
    Collapse 47 calorimeter layers -> 16 channels.
      channels 0-14 : sum of 3 consecutive layers  (layers 0-44, groups of 3)
      channel  15   : sum of remaining layers 45-46
    Input  shape: (H, W, 47)
    Output shape: (H, W, 16)
    """
    h, w = img.shape[:2]
    out = np.zeros((h, w, 16), dtype=np.float32)
    # First 45 layers -> 15 channels (each = sum of 3)
    out[:, :, :15] = img[:, :, :45].reshape(h, w, 15, 3).sum(axis=3)
    # Remaining 2 layers -> 1 channel
...
def process_root_file(in_file, prefix='evt', start_index=0, n_events=-1,
                      ...
                      base_dir='data', adc_scale=0.04,
                      noise_floor=4.0, adc_max=1_000_000,
                      fixed_box=True, fixed_w=0.034249, fixed_h=0.107281,
                      seed=42):

    dim1, dim2 = 736, 736

    # ── Physical ranges (must match image generation) ─────────────────────
    eta_0 = 1.4
    eta_n = 3.1
    phi_0 = -3.1416
    phi_n =  3.1416
    ...
    print(f"ADC    : scale={adc_scale}, noise_floor={noise_floor}, adc_max={adc_max}")
    ...
    # ── Load generator-level info (eta, phi, energy) ──────────────────────
    ...
    for evt in range(n_events):
        ...
        # Wrap phi from [0, 2π] -> [-π, +π] if necessary
        if gen_phi_val > np.pi:
            gen_phi_val -= 2.0 * np.pi

        # ── Continuous YOLO centre coordinates from gen η/φ ──────────────
        x_center = (gen_phi_val - phi_0) / (phi_n - phi_0)   # phi -> x
        y_center = (gen_eta_val - eta_0) / (eta_n - eta_0)   # eta -> y
        ...
        # ── Log-scale ADC compression -> uint8 ───────────────────────────
