...
void addPileupToSignal(const char* signalFile = "Electron_Pt_025_Eta_170_Events_2K_PU_000_Set01_Step2.root",
    ...
    const double MIP_MeV = 0.90;  // 0.9 MeV 
    ...
    for (int sigEvt : sigEventList) {
        ...
        // ============ Validation Tree Computation ============
        // Initialize arrays
        for (int l = 0; l < N_layers; l++) {
            NPix[l] = 0;
            EMin[l] = 1e9;  // Large value for finding minimum
            EMax[l] = 0.0;
            ETotal[l] = 0.0;
        }
        
        // Process combined hits
        for (auto& kv : etaPhiHitMap) {
        ...
        // Fix EMin for layers with no hits (set to 0)
        for (int l = 0; l < N_layers; l++) {
            if (EMin[l] > 1e8) {
                EMin[l] = 0.0;
            }
        ...
        val_event_id = sigEvt;
        valTree->Fill();
        // ============ End Validation Tree Computation ============
