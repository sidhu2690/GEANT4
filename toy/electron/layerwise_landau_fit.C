...
void layerwise_landau_fit() {
    ...
    for (Int_t iLayer = 0; iLayer <= maxLayer; iLayer++) {
        ...
        TF1 *fit = new TF1(Form("fit%d", iLayer), "landau", 0, 1);
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        ...
        if (layer >= 0 && edep > 0) {
            if (mpvPerLayer.find(layer) != mpvPerLayer.end()) {
                Double_t cutThreshold = 0.0 * mpvPerLayer[layer];
    ...
    grHits->SetTitle("Simhit Distribution Profile (Electron Primary);Layer Number;Average Number of Simhits per Particle");
    ...
    grEnergy->SetTitle("Energy Deposition Profile (Electron Primary);Layer Number;Average Energy Deposited per Particle (MeV)");
