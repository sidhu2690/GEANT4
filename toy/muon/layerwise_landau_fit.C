...
void layerwise_landau_fit() {
    ...
    for (Long64_t i = 0; i < nentries; i++) {
        ...
        if (layer >= 0 && edep > 0) {
            if (mpvPerLayer.find(layer) != mpvPerLayer.end()) {
                Double_t cutThreshold = 0 * mpvPerLayer[layer];
    ...
    grHits->SetTitle("Simhit Distribution Profile (Muon Primary);Layer Number;Average Number of Simhits per Particle");
    ...
    grEnergy->SetTitle("Energy Deposition Profile (Muon Primary);Layer Number;Average Energy Deposited per Particle (MeV)");
