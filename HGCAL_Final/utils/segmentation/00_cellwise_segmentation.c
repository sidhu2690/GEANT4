...
void cellwise_segmentation(const char* inputFile = "Positron_Pt_200_Eta_270_Events_2K_PU_000_Set01_Step1.root",
                            const char* outputFile = "Positron_Pt_200_Eta_270_Events_2K_PU_000_Set01_Step2.root") {
    ...
    cellTree->SetAutoSave(0);  // Disable autosave
    cellTree->SetAutoFlush(50000);  // Flush every 50k entries
    ...
    while (reader.Next()) {
        ...
        if (processedCount % 50000 == 0) {
            std::cout << "Processed " << processedCount << " / " << nEntries 
                      << " (written: " << writtenCount << " cells)" << std::endl;
        }
    ...
    std::cout << "Total cells written: " << writtenCount << std::endl;

    // FORCE FINAL WRITE
    std::cout << "Flushing tree to disk..." << std::endl;
    cellTree->FlushBaskets();
    cellTree->Write("", TObject::kOverwrite);
    
    Long64_t cellEntries = cellTree->GetEntries();

    std::cout << "\n=== Output Summary ===" << std::endl;
    std::cout << "  1. GeneratorInfo: " << genEntries << " entries" << std::endl;
    std::cout << "  2. CellWiseSegmentation: " << cellEntries << " entries" << std::endl;
    std::cout << "======================\n" << std::endl;
